...
app.put('/api/clients/:id', isAuthenticated, isAdmin, async (req, res) => {
    const { id } = req.params;
...
app.post('/api/clients/:id/logo', isAuthenticated, isAdmin, upload.single('logo'), async (req, res) => {
    const { id } = req.params;
...
app.post('/api/users/:id/photo', isAuthenticated, upload.single('photo'), async (req, res) => {
    const { id } = req.params;
    if (!req.file) return res.status(400).json({ error: 'No se subió ningún archivo' });
